...
public interface LecturerRepository extends JpaRepository<Lecturer, Long> {
    ...
    @Query("SELECT DISTINCT l FROM Lecturer l " +
            ...
            "OR (:payRateLower IS NULL OR :payRateUpper IS NULL OR (l.hourlyPayRate BETWEEN :payRateLower AND :payRateUpper)) " +
            "OR (:qualification IS NULL OR LOWER(q.level) LIKE LOWER(CONCAT('%', :qualification, '%'))) " +
            "OR (:isAssigned IS NULL OR l.isAssigned = :isAssigned) " +
            "OR (:language IS NULL OR FUNCTION('FIND_IN_SET', :language, l.language) > 0) " +
            "OR (:globalSearch IS NULL OR LOWER(l.name) LIKE LOWER(CONCAT('%', :globalSearch, '%')) " +
            "OR LOWER(l.preference) LIKE LOWER(CONCAT('%', :globalSearch, '%')) " +
            "OR LOWER(l.fieldOfWork) LIKE LOWER(CONCAT('%', :globalSearch, '%')))")
